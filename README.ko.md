...
**수정 사항:**
* **비디오 역추론 오류 수정**: 일부 모델(예: Qwen3.5-Plus)이 다중 이미지 분석을 지원하지 않는 것으로 오인되어 오류가 발생하던 문제를 수정했습니다. 이제 모델별 프레임 제한을 지능적으로 추론하고 자동 절단을 지원하여 작업 중단을 방지합니다.
* **서브그래프 마운트 최적화**: Node 2.0 (Vue) 및 LiteGraph 모드에서 서브그래프(Subgraph) 노드의 어시스턴트 초기화 및 안정성 문제를 수정했습니다. 동일한 이름의 여러 입력창에 대한 정밀 매칭을 지원합니다.
