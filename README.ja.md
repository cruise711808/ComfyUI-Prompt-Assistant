...
**修正内容:**
* **ビデオキャプションの修正**: 一部のモデル（Qwen3.5-Plusなど）がマルチ画像分析をサポートしていないと誤って識別される問題を修正しました。フレーム制限のインテリジェントな推論と自動切り捨て機能を追加し、タスクの中断を防ぎます。
* **サブグラフマウントの最適化**: Node 2.0 (Vue) および LiteGraph モードでのサブグラフノードにおけるアシスタントの初期化と安定性を修正しました。複数の同名入力に対する正確なマッチングをサポートしました。
